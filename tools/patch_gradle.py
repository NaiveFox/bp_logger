...
"""
Поддержка обеих схем Gradle: Groovy (build.gradle) и Kotlin DSL (build.gradle.kts).
...
- coreLibraryDesugaring включён, desugar_jdk_libs 2.0.4
...
""")
        return
    txt = ptext(f)
    def bump(block: str) -> str:
        block = re.sub(r'id\s+"com\.android\.application"\s+version\s+"[^"]+"\s+apply\s+false',
                       f'id "com.android.application" version "{AGP_VERSION}" apply false', block)
        block = re.sub(r'id\s+"org\.jetbrains\.kotlin\.android"\s+version\s+"[^"]+"\s+apply\s+false',
                       f'id "org.jetbrains.kotlin.android" version "{KOTLIN_VERSION}" apply false', block)
        return block
    txt = re.sub(r"plugins\s*\{[\s\S]*?\}", lambda m: bump(m.group(0)), txt, count=1)
    wtext(f, txt)
...
""")
    ...
    txt = ptext(f)
    def bump(block: str) -> str:
        block = re.sub(r'id\("com\.android\.application"\)\s+version\s+"[^"]+"\s+apply\s+false',
                       f'id("com.android.application") version "{AGP_VERSION}" apply false', block)
        block = re.sub(r'id\("org\.jetbrains\.kotlin\.android"\)\s+version\s+"[^"]+"\s+apply\s+false',
                       f'id("org.jetbrains.kotlin.android") version "{KOTLIN_VERSION}" apply false', block)
        return block
    txt = re.sub(r"plugins\s*\{[\s\S]*?\}", lambda m: bump(m.group(0)), txt, count=1)
    wtext(f, txt)
...
""")
    ...
    # compileOptions with desugaring
    if "compileOptions" in txt:
        txt = re.sub(r"compileOptions\s*\{[\s\S]*?\}",
                     """compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
...
def main():
    ...
    root_groovy = app_dir / "android" / "build.gradle"
    root_kts    = app_dir / "android" / "build.gradle.kts"
    app_groovy  = app_dir / "android" / "app" / "build.gradle"
    app_kts     = app_dir / "android" / "app" / "build.gradle.kts"

    # root
    if root_kts.exists() or (not root_groovy.exists() and not root_kts.exists() and app_kts.exists()):
        patch_root_kts(root_kts)
    else:
        patch_root_groovy(root_groovy)
    ...
    if app_kts.exists():
        ...
        # Если ничего нет — предпочтём KTS как современный вариант
        patch_app_kts(app_kts)

    print("✅ Patched for AGP 8.3.2 / Gradle 8.6 / Kotlin 1.9.24 / Java 17 with desugaring (Groovy/KTS).")
